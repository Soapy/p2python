...
from p2python.protocol import PeerConnection, REQUEST_SIZE
from p2python.tracker import Tracker
    ...
    async def start(self):
        ...
        while True:
            if self.piece_manager.complete:
                logging.info('Torrent downloaded...')
                break
            if self.abort:
...
class PieceManager:
    """
    Keeps track of all available pieces for the peers.
    """
    def __init__(self, torrent):
    ...
    def remove_peer(self, peer_id):
        """
        If a peer connection is dropped, it will remove the peer.
        """
        if peer_id in self.peers:
