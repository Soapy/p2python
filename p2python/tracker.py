...
class TrackerResponse:
    ...
    def incomplete(self) -> int:
        """
        Number of leechers on the current torrent
        """
    ...
    def peers(self):
        """
        A list of tuples with each peer being structured as (ip, port)
        """
...
class Tracker:
    ...
    under download or seeding state (seeding not implemented yet)
    """
    ...
    async def connect(self,
        ...
        async with self.http_client.get(url) as response:
            if not response.status == 200:
                raise ConnectionError('Unable to connect to tracker: code {}'.format(response.status))
